...
protected:
	...
	virtual void loadScene() override
	{
		...
		auto* sun = new SphereMeshComponent(shaderProgram, sunMat, 2.0f);
		...
		// Add the empty spin GO to the sun
		this->addChildGameObject(earthOrbitGO);
		earthOrbitGO->addComponent(new SpinComponent(100.0f));
		...
		earthGO->addComponent(new SpinComponent(100.0f));
		...
		auto* earth = new SphereMeshComponent(shaderProgram, earthMat, 1.0f);
		...
		moonOrbitGO->addComponent(new SpinComponent(-120.0f));

		auto* moonGO = new GameObject();
		moonOrbitGO->addChildGameObject(moonGO);
		moonGO->setPosition(vec3(2.0f, 0.0f, 0.0f), LOCAL);
		moonGO->addComponent(new SpinComponent(-120.0f));
		...
		cameraObject1->setPosition(vec3(10.0f, 0.0f, 0.0f));
		cameraObject1->setRotation(glm::rotate(PI_OVER_2, UNIT_Y_V3), LOCAL);
